...
def main(route_state_rows, completed_workflow_add):
    try:
        ...
        workflow_to_add = completed_workflow_add.strip() if isinstance(completed_workflow_add, str) else completed_workflow_add
        if workflow_to_add not in {"", "WF-09"}:
            raise ValueError("completed_workflow_add 只能为空或 WF-09")
        ...
        return {
            "merge_ok": True,
            "error_message": "",
            "completed_workflows_json": json.dumps(clean_completed, ensure_ascii=False, separators=(",", ":")),
