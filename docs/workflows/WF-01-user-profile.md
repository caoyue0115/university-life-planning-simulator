...
主 Agent 在首次建档、查看/修改画像时调用。输入 `AGENT_USER_INPUT`、`user_id`、`session_id`；读取/写入 `user_profile`。输出 `data.profile_json`。能力标签分别保存 `self_reported`、`agent_inferred`、`behavior_verified`；成绩、预算、家庭支持允许跳过并只保存粗粒度区间。
...
| 节点 | 输入 | 配置/分支 | 输出 |
|---|---|---|---|
| 读取已有画像 | `user_id` | 条件 `user_id=当前 user_id, record_key=user_profile` | `old_profile_json`，无记录=`null` |
...
| 校验身份与字段 | `user_id`、提取结果 | 身份缺失或 JSON 无效走错误；可跳过字段不算错误 | `valid` |
| 判断用户动作 | `action` | `confirm` 仅限明确确认；`modify/draft` 走展示 | 分支 |
| 写入画像 | `user_id`,`profile_json` | 逻辑键 `user_profile`，版本递增 | `write_result` |
...
- 失败：去掉 `user_id` 或模拟数据库报错，应为 `missing_user_id/write_failed`，回复不得含“已保存”。非 JSON 只重试一次。
...
正式版必须同时读取 `user_profile` 和 `pending_profile_draft`。草稿分支用“数据库”保存 `{user_id,profile_json,confirmation_token,record_version}` 后，经“决策”检查写入再结束；下一轮确认先读取该草稿，校验令牌对应当前版本，再把**已保存的草稿**写入 `user_profile`，不能从“确认保存”四个字重新生成画像。令牌生成能力以当前编辑器为准；无唯一值能力时降级为 `session_id + record_version`。数据库不可用时用长期记忆键 `user_id:pending_profile_draft`。
...
逐边映射：开始→读取传 `user_id`；读取→识别传 `old_profile_json,pending_profile_json,confirmation_token,AGENT_USER_INPUT`；提取→结构决策传 `profile_json,action,token_match`；草稿边传 `profile_json,user_id,session_id,record_version`；确认边传 `pending_profile_json,user_id`；每个写入→检查传对应 `write_result`。
