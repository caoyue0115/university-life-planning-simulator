...
flowchart TD
    ...
    N06 --> N07{"N07 决策：模型结果有效?"}
    N07 -->|否| N29["N29 消息：画像解析失败"] --> N30
    N07 -->|是| N08{"N08 分支器：requested_action"}
    N08 -->|draft 或 modify| N09["N09 代码：准备 pending 草稿和 token"]
    N09 --> N10{"N10 决策：已有画像记录?"}
    N10 -->|是| N11["N11 数据库：更新 pending 草稿"] --> N12{"N12 决策：更新 pending 成功?"}
    N10 -->|否| N13["N13 数据库：新增 pending 草稿"] --> N14{"N14 决策：新增 pending 成功?"}
...
| 决策 | 9 | N02、N07、N10、N12、N14、N18、N20、N22、N24 |
| 分支器 | 1 | N08 |
