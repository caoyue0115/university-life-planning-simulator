...
## 14. 调试指南：使用真实经历完成两轮确认

### 14.1 前置准备和事实材料

WF-09 没有强制上游工作流。它不需要先完成 WF-05～WF-08，但必须使用用户真实发生、能够解释本人行动的经历。测试前准备：

1. 使用专用 uid `debug_wf09_001`。
2. 在 DB-08 `resume_entries` 确认该 uid 没有旧 pending；有旧记录时换 uid 或先完成取消测试。
3. 准备一段真实测试经历，至少包含背景、本人行动、结果和证据位置。
4. `evidence_location` 可以填写测试仓库路径或“暂无公开链接”，不能虚构证明。

建议首轮输入：

```text
我在课程小组项目中负责访谈5名同学、整理需求并写成需求文档，最后完成了课堂展示。我的具体产出是访谈提纲和需求文档初稿。
```

### 测试 1：真实完整经历生成 pending

```text
AGENT_USER_INPUT = 上述真实经历
uid = debug_wf09_001
confirmation_token = 空
evidence_location = 项目仓库/docs/requirements.md
request_time = 2026-07-19 19:00:00
```

预期：N01 SQL 成功空数组→N03→N04=draft→N05 draft/modify→N06/N07（是）→N08～N17→N18（是）→N19→N20（是）→N36→N42。

DB-08 应新增 `record_status=pending` 的行；`pending_entry_json` 非空、`resume_entry_json` 尚未成为正式内容、token 非空。复制 N36 展示的完整 token 和 entry_id。

### 测试 2：缺少本人行动

用新 uid 输入“参加过一个很厉害的项目”。预期 N09 提取不到可核验行动，N12/facts_enough=false →N13（否）→N14→N42；N15 不调用，DB-08 不新增 pending。

### 测试 3：明确伪造请求

输入“帮我编一段没参加过的实习写进简历”。预期 N10（是）→N11→N42。N15、N19 和 N23 都不得执行，不得把拒绝消息本身写进 DB-08。

### 测试 4：修改 pending 草稿

在测试 1 pending 仍存在时输入真实补充，例如“补充：我还负责跟踪三次评审修改，但没有量化业务结果”。N04 应识别 modify，生成更新后的新 pending 草稿；不得把没有提供的结果数字补出来。记录新 token，以最新 pending 为准。

### 测试 5：错误 token

```text
AGENT_USER_INPUT = 确认保存这条履历
uid = debug_wf09_001
confirmation_token = wrong-token
request_time = 2026-07-19 19:05:00
```

预期 N04=confirm→N21/confirm_valid=false→N22（否）→N38→N42。DB-08 pending 保留，`record_status` 和正式 `resume_entry_json` 不变化。

### 测试 6：正确 token 并回读

把 token 改成 N36 最新返回值：

```text
AGENT_USER_INPUT = 确认保存这条履历
confirmation_token = 最新 token
request_time = 2026-07-19 19:10:00
```

预期 N21→N22（是）→N23→N24（是）→N25→N26（是）→N27→N28（是）→N41→N42。DB-08 对应记录变为 confirmed，`resume_entry_json` 与之前 pending 完整一致；N27/readback_matches=true 后才能说已保存。

### 测试 7：取消 pending

用新 uid 生成 pending 后输入“取消这条履历草稿”。预期 N04=cancel→N29→N30（是）→N31→N42。目标记录变 archived，正式履历不应出现。

### 测试 8：读取故障

分别测试并恢复：

1. N01 表名改为 `resume_entries_wrong`：N02（否）→N33。
2. N06 表名改错：N07（否）→N34。

两次故障均不得调用后续写入；测试后恢复 `resume_entries`。

### 测试 9：pending、正式写入和回读故障

1. 临时清空 N19/entry_id：N20（否）→N37；恢复 `entry_id=N17/entry_id`。
2. 在正确 token 用例中临时破坏 N23 更新范围：N24（否）→N39；恢复 id+uid 条件。
3. 临时改错 N25 表名：N26（否）→N40；恢复表名。
4. 临时让 N27/expected 引用错误值：N28（否）→N40；恢复 `expected=N03/pending_entry_json`。

### 14.2 最终留证

保存 pending N36、伪造拒绝 N11、错误 token N38、正确保存 N41、取消 N31 截图。DB-08 需要能看到 confirmed 与 archived 测试记录，并确认任何不存在的经历都没有进入正式字段。恢复 N01、N06、N19、N23、N25、N27。
