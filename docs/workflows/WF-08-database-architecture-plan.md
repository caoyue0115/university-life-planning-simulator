...
不能使用 `state_version > 0`，也不能使用 WF-08 的 `review_version`。

#### 更新字段

| 公共路由表字段 | 值类型 | 引用来源 |
|---|---|---|
| `current_workflow` | 引用 | `WF08_C01_解析模型输出.current_workflow` |
| `current_status` | 引用 | `WF08_C01_解析模型输出.current_status` |
| `next_workflow` | 引用 | `WF08_C01_解析模型输出.next_workflow` |
| `completed_workflows` | 引用 | `WF08_C02_合并路由状态.completed_workflows_json` |
| `state_version` | 引用 | `WF08_C02_合并路由状态.state_version_next` |
...
1. `completed_workflows` 必须引用 C02，不能固定写成 `["WF-08"]`。
2. `state_version` 必须引用 C02 的递增结果。
3. 三个路由字段必须引用 C01。
4. WF-08 无权修改 `profile_status`。
5. 不得把复盘字段写入公共路由表。
6. 不得保存 `reply`。
...
- [ ] 没有更新 `profile_status`。
- [ ] 没有写入业务字段或 reply。
- [ ] 后续预留连接 `WF08_写入_last_reply`。

### 第 13 个节点：`WF08_写入_last_reply`
...
| 值类型 | 引用 |
| 引用来源 | `WF08_C01_解析模型输出.reply` |

本节点只执行以下一次赋值：

```text
last_reply = WF08_C01_解析模型输出.reply
```
...
- 因此不需要新增失败消息节点，也不需要新增第二个变量存储器。

#### 禁止事项与易错点

1. 不得直接引用大模型原始 `output`。
2. 不得把 DB07 的默认数据库输出当成用户回复。
3. 本节点只能写共享 `last_reply`，不能改公共路由状态或模块状态。
4. 复盘状态不存在分支的否则出口不得接到这里，必须先初始化并统一读取。
5. 不得修改现有 N90。
6. 不得为 WF-08 新建第二个结束节点。
...
WF-08 的业务基线、六字段模块表、十三个节点、数据库版完整提示词、C01 解析代码和 C02 路由合并代码均已完成讨论确认并归档。下一条工作流从 WF-09 履历素材模块开始。
