...
> 如果你想先用一个 MAIN 画布、不接数据库和知识库，验证 12 个模块的初步对话效果，请使用[大模型优先最小可运行版](../mvp/README.md)。本目录仍是面向正式持久化版本的详细教程。
...
WF-01～WF-12 的节点页面按[平台 UI 配置契约](PLATFORM-UI-CONTRACT.md)填写。数据库读取使用“自定义 SQL”，新增/更新使用“表单处理数据”；代码只使用 Python；条件判断使用“分支器”。总编排中的“工作流”调用节点尚缺配置页截图，因此本文件只规定其必须绑定的子流程、公共输入和统一输出，不编造未确认的按钮名称。
...
WF-01 已改成单参数入口。`调用_WF01_用户建档` 不能分别传 `uid` 和 `confirmation_token`，必须先把业务字段拼成一条 JSON 字符串，再作为它唯一的 `AGENT_USER_INPUT`：

```json
{
  "uid": "当前用户唯一标识",
  "user_input": "用户本轮原话",
  "confirmation_token": "上一轮 WF-01 token；首次为空",
  "request_time": "YYYY-MM-DD HH:mm:ss"
}
```

如果工作流调用节点只显示一个输入框，就把上面完整对象序列化后传入该输入框，不要尝试在子工作流开始节点恢复多个参数。`uid` 必须来自可信的登录身份、后端映射或平台用户标识，不能让大模型从用户原话中猜测。WF-02～WF-12 尚未逐个完成单参数改造时，不能据此声称 12 个子流程已经全部可被平台内部调用。
...
`读取用户状态` 使用“数据库 → 自定义 SQL”，输入区添加 `uid｜引用｜开始/uid`，查询 `user_profiles`、`main_plans`、`semester_tasks`、`session_recaps` 时都必须带 `WHERE uid='{{uid}}'`。节点固定输出 `isSuccess`、`message`、`outputList`；其后用代码节点或变量提取器分别整理出 `profile_json`、`main_plan_json`、`semester_tasks_json`、`session_recap_json`。尚未建数据库时，可改用 `长期记忆检索`，输入引用 `uid`，输出读取 `memory:Array<Object>`。
...
在 `按意图路由` 中点击“+ 添加分支”。每条分支的“引用变量”选择意图提取节点的 `intent`，“选择条件”选择等于，“比较类型”选择常量/直接输入（不要选引用），“比较值”依次填写下表的 `WF-01`～`WF-12`：
