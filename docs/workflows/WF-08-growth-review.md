...
| N21 | 输入 `reply=N11/reply`、`review=N17/stored_review_json`、`change=N11/pending_change_json`、`token=N11/confirmation_token`；回答中依次展示复盘、任务微调草稿和 token，并明确“DB-06 任务尚未修改；用户确认后把 change 交给 WF-07 执行” |
| N22 | 输入 `reply=N11/reply`、`review=N17/stored_review_json`、`change=N11/pending_change_json`、`token=N11/confirmation_token`；回答中依次展示复盘、路径切换建议和 token，并明确“DB-05 主规划尚未修改；用户确认后进入 WF-06 重新生成并确认主规划” |
...
## 10. 调试指南：准备 WF-06 主规划和 WF-07 任务证据

### 10.1 上游准备

1. 使用测试 uid `debug_wf08_001` 完成 WF-06 正确确认，取得唯一 active `plan_id`。
2. 用该 plan_id 在 WF-07 创建至少三条任务：一条 completed 且有 actual_evidence、一条 postponed 且有 delay_reason、一条 pending。
3. 在 DB-05 核验 active 主规划，在 DB-06 核验三条任务都属于同一 uid 和 plan_id。
4. 在 DB-07 `growth_reviews` 记录测试前行数。

WF-08 只读取和总结这些记录；它写 DB-07，但不会直接更新 DB-05 或 DB-06。任何消息中出现“建议调整”都不代表调整已经执行。

### 测试 1：有规划和任务的正常复盘

```text
AGENT_USER_INPUT = 帮我复盘最近一个月：需求文档已完成，课程冲突导致一项任务延期，另一项还没开始
uid = debug_wf08_001
plan_id = WF-06 active plan_id
request_time = 2026-07-19 18:00:00
```

预期：N01→N02（是）→N03/has_plan=true→N04（是）→N05→N06（是）→N07/evidence_enough=true→N08（是）→N09～N13→N14（是）→N15→N16（是）→N17→N18（是）→N19，再按 recommendation_type 到 N20/N21/N22，最后 N30。

DB-07 应新增一行并能由 N15 回读一致。`evidence_summary_json` 要区分 DB-06 证据和用户本轮自述；不能把推断写成事实。

### 测试 2：没有 active 主规划

使用不存在的 plan_id。N01 SQL 成功空数组 → N02（是）→ N03/has_plan=false → N04（否）→ N23 → N30。N05 不执行，DB-07 不新增。

### 测试 3：主规划 SQL 失败

临时把 N01 表名改为 `main_plans_wrong`。预期 N02（否）→N24→N30。恢复 `main_plans`。

### 测试 4：没有任务但用户提供足够详细事实

使用一个有 active 规划但 DB-06 无任务的测试 uid，输入至少 20 个字符的具体自述，例如“本月完成两次导师沟通并整理了三页研究笔记，但没有录入任务”。N07 可以令 evidence_enough=true；生成结果必须把这些标为“用户自述”，不能伪装成数据库完成记录。

### 测试 5：没有任务且输入过短

同样在任务为空时输入“帮我复盘”。预期 N07/evidence_enough=false → N08（否）→ N26 → N30，N09 不调用，DB-07 不写入。

### 测试 6：任务 SQL 失败

临时把 N05 表名改为 `semester_tasks_wrong`。预期 N06（否）→N25→N30。恢复表名。

### 测试 7：continue 建议

使用进展基本正常、证据完整的输入，观察 N11/recommendation_type=continue。预期 N19→N20；`pending_change_json` 应为 `{}`，token 为空，不修改 DB-05/DB-06。

### 测试 8：adjust 建议

使用“当前路径仍合适，但本月任务过多，需要减少并延期低优先级任务”的证据。若模型输出 adjust，预期 N19→N21。N21 只能展示 pending_change 和 token，并提示进入 WF-07；检查 DB-06 在此轮前后完全不变。

### 测试 9：consider_switch 建议

使用能够支持重新评估路径的具体事实。若输出 consider_switch，预期 N19→N22。N22 只建议进入 WF-06，检查 DB-05 active 行在此轮前后不变。

### 测试 10：复盘结构无效

临时让 N10 输出非法 recommendation_type 或无效 review_json。预期 N11/review_valid=false → N12（否）→N27，N13 不执行。恢复 N10 输出。

### 测试 11：写入和回读故障

1. 临时清空 N13/review_id：N14（否）→N28，不能说已保存；恢复 `review_id=N11/review_id`。
2. 临时把 N15 表名改错：N16（否）→N29；恢复 `growth_reviews`。
3. 临时让 N17/expected 引用错误值：N18（否）→N29；恢复 `expected=N11/review_json`。

### 10.2 最终核验

保存 N20、N21、N22 中实际跑到的分支截图，以及 N26 追问和 N28/N29 故障截图。DB-07 中确认 `review_id/plan_id/recommendation_type/review_json/evidence_summary_json` 完整；同时对比 DB-05/DB-06，证明 WF-08 没有直接修改它们。恢复 N01、N05、N10、N13、N15、N17。
